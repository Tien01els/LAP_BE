...
module.exports = {
    ...
    getDoTimeAssignmentOfStudent: async (req, res) => {
        try {
            const studentId = req.userId;
            const assignmentId = req.params.assignmentId;
            const result = await studentAssignmentService.findDoTimeAssignmentOfStudent(
                studentId,
                assignmentId
            );
            return res.status(result.statusCode).send(result.data);
    ...
    startAssignment: async (req, res) => {
        try {
            const assignmentId = req.params.assignmentId;
            const studentId = req.userId;
            const result = await studentAssignmentService.startAssignment(studentId, assignmentId);
            return res.status(result.statusCode).send(result.data);
        } catch (error) {
            const errorStatus = error.statusCode || 500;
            console.log(error);
